...
public final class IdentityGenerator {
    ...
    /**
     * Derives the key used to encrypt/decrypt local backups (see
     * backup.BackupExporter), from the same Ed25519 private seed as
     * everything else — so restoring a backup only ever needs the same 24
     * words you already have to protect anyway, no separate passphrase to
     * remember or lose. Tying it to the seed doesn't add a new leak
     * surface: anyone who has the seed already controls the identity
     * outright.
     */
    public static byte[] deriveBackupKey(Identity identity) throws GeneralSecurityException {
        return CryptoPrimitives.deriveKey(identity.privateKey, new byte[32], "nulchat-backup-key", 32);
    }
